...
fn main() -> Result<(), BacktestError> {
    ...
    println!(
        "Annualized vol    : {:>12.2}%",
        result.metrics.annualized_volatility * 100.0
    );
    ...
    println!(
        "Profit factor     : {:>12.2}",
        result.trade_stats.profit_factor
    );
    println!(
        "Win rate          : {:>12.2}%",
        result.trade_stats.win_rate * 100.0
    );
    println!("Round-trip trades : {:>12}", result.trade_stats.num_trades);
    println!("Fills             : {:>12}", result.fills);
    println!("Total fees        : {:>12.2}", result.total_fees);
    println!("Total slippage    : {:>12.2}", result.total_slippage);

    // Persist artifacts for downstream analysis.
    result.write_equity_csv("ma_crossover_equity.csv")?;
    result.write_trades_csv("ma_crossover_trades.csv")?;
    println!("\nWrote ma_crossover_equity.csv and ma_crossover_trades.csv");
    Ok(())
}
